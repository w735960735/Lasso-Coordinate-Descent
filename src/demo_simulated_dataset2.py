...
class MyLassoSimDataset:
    ...
    def create_simulated_data(self):
        """
        Data is siumlated using the formula:
        
        y = (-1) + (1 * X) + (2 * X**2) + (3 * X**3) + eps
        
        where X and eps are random numbers. Data is then standardized suing 
        StandardScaler().
    ...
    def plot_objectives(self,betas_cyc,betas_rnd,X_train,y_train):
        ...
        plt.title("Objective Value vs. Iteration (Lambda = 0.01043)")
        plt.legend()        
...
if __name__=='__main__':
    ...
    """
    print("Applying MyLasso algorithm on simulated Dataset:\n")
    print("Forumla used for simulation: y = (-1) + (1 * X) + (2 * X**2) + (3 * X**3) + eps")
    print("where X and eps are random numbers.\n")
    print("Both Cylic and Coordinate Descent algorithms are used.\n")
    print("Training process started ...", end = '')
    myalgo = MyLassoSimDataset()
    X_train, y_train, X_test, y_test = myalgo.create_simulated_data()
    betas_cyc = myalgo.cycliccoorddescent(X_train, y_train)
    betas_rnd = myalgo.randcoorddescent(X_train, y_train)
    print("Complete!!\n")
